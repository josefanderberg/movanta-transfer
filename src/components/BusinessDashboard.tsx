  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-card shadow-[0_30px_80px_-30px_rgba(0,0,0,0.7)]">
      <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
        ...
        <span className="rounded-full border border-white/15 bg-white/[0.04] px-3 py-1 text-[11px] font-medium text-metal">
        ...
        <div className="bg-card p-6">
          ...
          <ul className="mt-4 flex flex-col gap-3">
            {businessDashboardRequests.map((request) => (
              <li
                key={request.vehicle}
                className="flex items-center justify-between rounded-lg border border-white/10 bg-bg-secondary px-3.5 py-3 text-sm transition-colors duration-200 hover:border-white/20"
              >
                <div>
